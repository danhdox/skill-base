...
This skill provides a structured framework for analyzing authentication and session flows for security, UX, and resilience gaps. It encodes practical decision criteria, standard review checkpoints, and output conventions so teams can execute consistently across different AI agents and operators. The goal is to produce an actionable artifact that can be reused in downstream planning and execution.
...
```json
{
  "authentication_flow_review": {
    "artifact_type": "auth_review_report",
